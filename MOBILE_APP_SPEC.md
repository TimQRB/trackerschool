...
## 23. HC02 Device — детальная информация (из документации)

### 23.1 Физические характеристики
| Параметр | Значение |
|---|---|
| Форм-фактор | Электронный студенческий билет (карточка) |
| Защита | IP66 (пыль/вода) |
| Аккумулятор | 1000 мАч, полимерный |
| Время ожидания | 168 часов (7 дней) |
| Материал | Сенсорный тонкий лёгкий терминал |

### 23.2 Режимы позиционирования
Устройство поддерживает 9 режимов:
1. Beidou (北斗)
2. GPS
3. GLONASS
4. AGPS
5. Wi-Fi позиционирование
6. Базовые станции (LBS)
7. Датчик движения
8. Алгоритмы ИИ
9. 2.4 ГГц RFID (для посещаемости)

**Для мобильного приложения**: на карте видно только итоговые координаты (lat/lon), которые приходят в пакете 0x03E1. Режим (`mode`) может быть: `2` — базовые станции, `3` — Wi-Fi, `4` — GPS.

### 23.3 Кнопки и жесты устройства
| Кнопка | Действие |
|---|---|
| SOS (удержание 3с) | Вызов SOS-номера |
| Кнопка 1 (удержание 3с) | Вызов контакта 1 |
| Кнопка 2 (удержание 3с) | Вызов контакта 2 |
| Кнопка 3 (удержание 3с) | Вызов контакта 3 |
| При включённом — удержание 6с | Выключение |
| При включённом — удержание 15с | Сброс |
| При выключенном — удержание 3с | Включение |
| Во время разговора + | Громче |
| Во время разговора − | Тише |

### 23.4 Интервалы отчётов по умолчанию
| Тип пакета | Протокол | Интервал |
|---|---|---|
| Heartbeat (батарея) | 0x03DC | 300 сек (5 мин) |
| Позиция | 0x03E1 | Настраивается (default ~60-300 сек) |
| Пульс | 0x105E | 60 мин |
| SpO₂ | 0x1063 | По запросу пользователя |

### 23.5 Посещаемость через RFID
Устройство имеет модуль **2.4 ГГц RFID**, который используется для:
- Автоматической отметки входа в школу
- Автоматической отметки выхода из школы
- Передача данных о посещаемости на платформу

В текущей реализации посещаемость отслеживается через **геозоны** (enter_school_zone → отметка входа, exit_school_zone → отметка выхода). При наличии RFID-считывателя в школе можно добавить отдельный обработчик.

### 23.6 Схема топологии (из документации)
```
Устройство (HC02)
    │
    ├── Регистрационный центр (порт 13000) — однократно при первом включении
    │       └── получает адрес Service Node
    │
    └── Service Node (порт 13001) — постоянное соединение
            ├── Link (0x0000) — привязка к серверу
            ├── Heartbeat (0x03DC) — батарея/статус
            ├── Position (0x03E1) — GPS координаты
            ├── Alarm (0x03DB) — SOS/вкл/выкл
            ├── Heart Rate (0x105E) — пульс + шаги
            ├── Blood Oxygen (0x1063) — SpO₂
            ├── Call Log (0x0312) — история звонков
            └── SMS Report (0x1016) — полученные SMS
```

---

## 24. Приложение НЕ делает (границы ответственности)
